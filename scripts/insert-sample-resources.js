// This script inserts sample resources directly using the Supabase client
...
async function insertSampleResources() {
  ...
  try {
    // First, check if the type column exists, if not, add it
    const { data: tableInfo, error: tableError } = await supabase
      .from('educational_resources')
      .select('type')
      .limit(1);
    
    if (tableError && tableError.message.includes('column "type" does not exist')) {
      console.log('Adding type and metadata columns...');
    ...
    console.log(`Successfully inserted ${data.length} sample resources`);
